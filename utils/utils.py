...
def print_confusion_matrix(y_true, y_pred, title):
    ...
    for row in conf_mat:
        print(" ".join(f"{num:4}" for num in row))  # 统一宽度，确保对齐
...
def clustering_accuracy_score(y_true, y_pred, known_lab):
    ...
    old_acc = 0
    total_old_instances = 0
    known_mask = np.isin(y_true, known_lab)  # 已知类别的掩码
    for i in known_lab:
        old_acc += w[ind_map[i], i]
        total_old_instances += sum(w[:, i])
    old_acc /= total_old_instances
    ...
    return (round(acc * 100, 2), round(old_acc * 100, 2), round(new_acc * 100, 2)), known_mask, novel_mask



def clustering_score(y_true, y_pred, known_lab):
    (acc_all, acc_known, acc_novel), known_mask, novel_mask = clustering_accuracy_score(y_true, y_pred, known_lab)
    ...
    # 打印已知类别的混淆矩阵
    print_confusion_matrix(y_true[known_mask], y_pred[known_mask], "Known Classes")

    # 打印新类别的混淆矩阵
    print_confusion_matrix(y_true[novel_mask], y_pred[novel_mask], "Novel Classes")
